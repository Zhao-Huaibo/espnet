...
from espnet.nets.beam_search_dual_transducer_online import (
...
class Speech2Text:
    ...
    def __init__(
        ...
        # asr_train_config = "/mnt/aoni04/jsakuma/development/espnet-g05-1.8/egs2/wsj/asr1/exp/asr_train_asr_cbs_transducer_8_4_4_raw_en_bpe80/config.yaml"
        # asr_model_file = "/mnt/aoni04/zhao/g11work/espnet/egs2/wsj/asr1/exp/asr_train_asr_cbs_transducer_multi3_eou_raw_en_bpe80/valid.loss_transducer.ave_10best.pth"
        ...
        beam_search = BeamSearchParallelTransducer(
            asr_model.decoder,
        ...
        # if look_ahead / hop_size == 3:
        #     self.is_5step = False
        #     print('4steps')
        # elif look_ahead / hop_size == 4:
        #     self.is_5step = True
        #     print('5steps')
        # else:
...
#         )
        ...
        self.encoder_states_r = None
        self.encoder_states_rr = None
...
#                 speech = torch.cat([speech, pad], dim=0)
    ...
    def streaming_decode(
        ...
        enc_out = enc_out[0][0]
        et_enc = time.time()
        enc_time = et_enc - st_enc
        st_dec = time.time()
        nbest_hyps = self.beam_search(enc_out, enc_out_r, is_final=is_final)

        if is_final:
            self.reset_inference_cache()
            
        return (nbest_hyps[0], nbest_hyps[1]), enc_time, st_dec
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            try:
                ...
                _end = 0
                logging.info("streaming with {} steps".format(_steps))

                for i in range(_steps):                        
                    _end = (i + 1) * speech2text._raw_ctx

                    final_hyps, _, _ = speech2text.streaming_decode(
